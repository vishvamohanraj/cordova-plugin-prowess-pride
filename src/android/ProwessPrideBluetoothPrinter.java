 ...
 */
public class ProwessPrideBluetoothPrinter extends CordovaPlugin {

    // actions
	private static final String ACTIVATE_PRINTER_LIBRARY = "activatePrinterLibrary";
	private static final String SEND_DATA_FOR_PRINT = "sendDataForPrint";
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals(LIST)) {
            ...
            listBondedDevices(deviceList,callbackContext);

        }else if (action.equals(SEND_DATA_FOR_PRINT)) {
            sendDataForPrint(callbackContext,args);

        }else if (action.equals(ACTIVATE_PRINTER_LIBRARY)) {
            activatePrinterLibrary(callbackContext);
        	...
        	JSONArray deviceList = new JSONArray();
            discoverUnpairedDevices(deviceList,callbackContext);
	 ...
	 */
	private void activatePrinterLibrary(CallbackContext callbackContext) {
		if(!ActivatePrinterLibrary.IS_Activate)
		{
			Intent intent=new Intent(cordova.getActivity(),ActivatePrinterLibrary.class);
			cordova.getActivity().startActivity(intent);
		}
	}
    ...
    private void listBondedDevices(JSONArray deviceList, CallbackContext callbackContext) throws JSONException {
    ...
    public void sendDataForPrint(final CallbackContext callbackContext, final CordovaArgs args)
	{
	        ...
	        public void run() {
	        	try 
	        	{
	        		...
	        		if (bluetoothSerialService.getState() == BluetoothService.STATE_CONNECTED) {
		    			...
		    			{
		    				ptrGen.iFlushBuf();
		    				if(args.getJSONArray(0).length()>0)
		    				{
    ...
    public String msgReturnByPrinter(Integer result) {
		...
		} else if (result == Printer_GEN.SUCCESS) {
			msgReturnByPrinter="Print Successfully";
		} else if (result == Printer_GEN.PLATEN_OPEN) {
			msgReturnByPrinter="Platen open";
		} else if (result == Printer_GEN.PAPER_OUT) {
			msgReturnByPrinter="Paper out";
		} else if (result == Printer_GEN.IMPROPER_VOLTAGE) {
			msgReturnByPrinter="Printer at improper voltage";
		} else if (result == Printer_GEN.FAILURE) {
			msgReturnByPrinter="Printing failed";
		} else if (result == Printer_GEN.PARAM_ERROR) {
			msgReturnByPrinter="Parameter error";
		}else if (result == Printer_GEN.NO_RESPONSE) {
			msgReturnByPrinter="No response from Pride device";
		}else if (result== Printer_GEN.DEMO_VERSION) {
			msgReturnByPrinter="Library in demo version";
		}else if (result==Printer_GEN.INVALID_DEVICE_ID) {
			msgReturnByPrinter="Connected  device is not authenticated.";
		}else if (result==Printer_GEN.NOT_ACTIVATED) {
			msgReturnByPrinter="Library not activated";
		}else if (result==Printer_GEN.NOT_SUPPORTED) {
